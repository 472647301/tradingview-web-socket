...
export class DataPulseProvider {
	...
	public _updateData(result: GetBarsResult): void {
		...
		for (const listenerGuid in this._subscribers) { // tslint:disable-line:forin
			...
			this._updateDataForSubscriber(listenerGuid, result)
				.then(() => {
					this._requestsPending -= 1;
	...
	private _updateDataForSubscriber(listenerGuid: string, result: GetBarsResult): Promise<void> {
		...
		return new Promise<GetBarsResult>(resolve => {
			resolve(result)
		}).then((result) => {
